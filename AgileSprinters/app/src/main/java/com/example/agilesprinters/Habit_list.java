...
/**
 * This is a class that keeps track of a list of habit objects planned for the day
 */
public class Habit_list {
    ...
    public void addToday(Habit habit, String date, String day) throws ParseException {
        ...
        if (habits.contains(habit)) { throw new IllegalArgumentException(); }
        if (sdf.parse(habitStartDate).after(sdf.parse(date))) { throw new IllegalArgumentException(); }
        if (!habit.getWeekdays().get(day)) { throw new IllegalArgumentException(); }
